<script>
  ...
  const handleLoad = async (id) => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_BASE_URL}/rcms-api/3/form/${id}`
    );
    data = await res.json();
  ...
  const handleFormSubmit = async (id) => {
    ...
    try {
      const submit = await fetch(
        `${import.meta.env.VITE_BASE_URL}/rcms-api/3/form?id=${id}`,
        {
          method: "POST",
          body: JSON.stringify({
            ...body,
    ...
    >
      {#each Object.entries(tabList) as [tab, form_id]}
        <a
          ...
          aria-selected="true"
          on:click={() => handleLoad(form_id)}>{tab}</a
        >
      {/each}
    </div>
  </div>
  ...
  <div class="col-lg-4 col-12 my-4">
    <div class="tab-content" id="v-tabs-tabContent">
      {#each Object.entries(tabList) as [tab, form_id], index}
        <div
          class="tab-pane fade row justify-content-center {activeTab === tab
            ? 'show active'
            : ''}"
          id="v-tabs-{tab}"
          role="tabpanel"
          ...
          <div class="card">
            <div class="card-body justify-content-center">
              {#if !message && !error && data}
                ...
                <form
                  on:submit|preventDefault={() => handleFormSubmit(form_id)}
                  bind:this={refs[form_id]}
                >
                  ...
                  <button
                    type="submit"
                    class="btn btn-primary btn-block mb-4 col-6">Send</button
                  >
                </form>
              {:else if message}
                <h4>{message}</h4>
                <button
                  ...
                  class="btn btn-primary btn-block mb-4 col-6"
              ...
              {:else if error}
                ...
                <button
                  on:click={() => (error = "")}
                  class="btn btn-primary btn-block mb-4 col-6">Go back</button
                >
              {:else}
                <FormPlaceholder />
              {/if}
